...
if st.button("📈 Start Training"):
    ...
    torch.save(class_names, 'class_names.pth')  # save class names
...
if uploaded_file is not None:
    ...
    if os.path.exists('class_names.pth'):
        class_names = torch.load('class_names.pth')
    else:
        st.error("❌ Class names not found. Please train the model first.")
    
    num_classes = len(class_names)
    model = models.resnet18(weights=None)
    model.fc = nn.Linear(model.fc.in_features, num_classes)
    model.load_state_dict(torch.load('fish_classification_resnet18.pth', map_location='cpu'))
    model = model.to('cpu')
    model.eval()
